(function () {
  ...
  const ICONS = {
    ...
    chevronDown: '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>',
  ...
  const I18N = {
    en: {
      editSection: 'Edit this section', sectionNotFound: 'Section not found',
      ...
      loading: 'Loading...', noHistory: 'No history',
      preview: 'Preview', restore: 'Restore', delete: 'Delete',
      ...
      deleteAll: 'Delete all', deleteAllConfirm: 'Delete all history for this page?',
      diffTitle: 'Diff (left: history {0} / right: current)', diffHistory: 'History', diffCurrent: 'Current',
      diffTooLarge: 'File too large to show diff.',
      noResults: 'No results', resultsCount: '{0} results',
      fullDraftFound: 'Unsaved draft found for full page edit.\n\n[OK] Resume editing\n[Cancel] Discard draft',
      sectionDraftFound: 'Unsaved draft found for section \"{0}\".\n\n[OK] Resume editing\n[Cancel] Discard draft',
      imageDrop: 'Drop image here', uploadFailed: 'Upload failed: ', uploadError: 'Upload error: ',
      backToTop: 'Back to top', copyLink: 'Copy link', dragToMove: 'Drag to move',
      justNow: 'just now', minAgo: 'min ago', hourAgo: 'hr ago', dayAgo: 'd ago', monthAgo: 'mo ago', yearAgo: 'yr ago',
    },
    ja: {
      editSection: 'このセクションを編集', sectionNotFound: 'セクションが見つかりませんでした',
      ...
      renameFailed: 'リネームに失敗しました: ', deleteFailed: '削除に失敗しました: ',
      createFailed: '作成に失敗しました: ',
      deleteConfirm: ' を削除しますか？',
      loading: '読み込み中...', noHistory: '履歴がありません',
      preview: 'プレビュー', restore: '復元', delete: '削除',
      restoreConfirm: ' の状態に復元しますか？',
      restored: '復元しました', restoreFailed: '復元に失敗しました',
      deleteHistoryConfirm: ' の履歴を削除しますか？',
      deleteHistoryFailed: '削除に失敗しました',
      deleteAll: '全履歴を削除', deleteAllConfirm: 'このページの全履歴を削除しますか？',
      diffTitle: '差分（左: 履歴 {0} / 右: 現在）', diffHistory: '履歴', diffCurrent: '現在',
      diffTooLarge: 'ファイルが大きすぎるため差分を省略しました。',
      noResults: '結果がありません', resultsCount: '{0}件の結果',
      fullDraftFound: 'ページ全体編集の保存されていない下書きがあります。\n\n[OK] 編集を続ける\n[キャンセル] 下書きを破棄',
      sectionDraftFound: '「{0}」セクションに保存されていない下書きがあります。\n\n[OK] 編集を続ける\n[キャンセル] 下書きを破棄',
      imageDrop: '画像をドロップ', uploadFailed: 'アップロード失敗: ', uploadError: 'アップロードエラー: ',
      backToTop: 'トップへ戻る', copyLink: 'リンクをコピー', dragToMove: 'ドラッグで移動',
      justNow: 'たった今', minAgo: '分前', hourAgo: '時間前', dayAgo: '日前', monthAgo: 'ヶ月前', yearAgo: '年前',
    },
  };
  function t(key, ...args) {
  ...
  function saveDraft(headingText, content) {
    try { localStorage.setItem(getDraftKey(headingText), content); } catch(e) {}
  }
  function loadDraft(headingText) {
    try { return localStorage.getItem(getDraftKey(headingText)); } catch(e) { return null; }
  }
  function clearDraft(headingText) {
    try { localStorage.removeItem(getDraftKey(headingText)); } catch(e) {}
  }
  ...
  function getContent() { return api(`/api/content?path=${encodeURIComponent(filePath)}`); }
  function saveContent(content) { return apiPost('/api/content', { path: filePath, content }); }
  function backupFile() { return apiPost('/api/backup', { filepath: filePath }); }
  function listBackups() { return api(`/api/backups?filepath=${encodeURIComponent(filePath)}`); }
  function getBackup(ts) { return api(`/api/backups/content?filepath=${encodeURIComponent(filePath)}&timestamp=${ts}`); }
  function restoreBackup(ts) { return apiPost('/api/restore', { filepath: filePath, timestamp: ts }); }
  function deleteBackup(ts) { return apiPost('/api/backups/delete', { filepath: filePath, timestamp: ts }); }
  function deleteAllBackups() { return apiPost('/api/backups/delete-all', { filepath: filePath }); }
  ...
  function splitSections(markdown) {
    ...
    for (const line of lines) {
      ...
      if (match) {
        ...
        current = { heading: line, level: match[1].length, title: match[2], lines: [line] };
      } else {
        if (!current) current = { heading: null, level: 0, title: '', lines: [line] };
  ...
  function addCopyButtons() {
    document.querySelectorAll('#article-body pre').forEach(pre => {
      ...
      if (pre.closest('pre') !== pre && pre.parentElement.closest('pre')) return;
      ...
      if (pre.scrollWidth > pre.clientWidth) {
        ...
        pre.addEventListener('scroll', () => {
          wrapper.classList.toggle('has-overflow-right', pre.scrollLeft + pre.clientWidth < pre.scrollWidth - 2);
  ...
  async function openSectionEdit(heading, btn) {
    ...
    if (sectionIndex === -1) { alert(t('sectionNotFound')); return; }

    const section = sections[sectionIndex];
    const sectionLevel = section.level;
    let endIndex = sectionIndex + 1;
    while (endIndex < sections.length) {
      if (sections[endIndex].heading && sections[endIndex].level <= sectionLevel) break;
      endIndex++;
    }
    const sectionMd = sections.slice(sectionIndex, endIndex).map(s => s.lines.join('\n')).join('\n');

    const existing = document.querySelector('.section-edit-area');
    if (existing) { closeSectionEdit(existing); }

    let nextEl = heading.nextElementSibling;
    while (nextEl) {
      const tag = nextEl.tagName ? nextEl.tagName.toLowerCase() : '';
      if (['h1','h2','h3','h4','h5','h6'].includes(tag) && parseInt(tag[1]) <= sectionLevel) break;
    ...
    saveBtn.className = 'btn btn-save'; saveBtn.textContent = t('save');
    const cancelBtn = document.createElement('button');
    cancelBtn.className = 'btn btn-cancel'; cancelBtn.textContent = t('cancel');
    ...
    const updatePreview = () => {
      if (window.marked) {
        ...
        preview.querySelectorAll('a[href]').forEach(a => { a.setAttribute('target', '_blank'); a.setAttribute('rel', 'noopener noreferrer'); });
      }
      ...
      previewTimer = setTimeout(async () => {
        try {
          ...
          if (res.html) {
            ...
            preview.querySelectorAll('pre').forEach(pre => { pre.style.position = 'relative'; });
            preview.querySelectorAll('a[href]').forEach(a => { a.setAttribute('target', '_blank'); a.setAttribute('rel', 'noopener noreferrer'); });
          }
        } catch(e) {}
      }, DEBOUNCE_PREVIEW);
    ...
    function syncScroll(source, target, isTextarea) {
      ...
      if (isTextarea) {
        ...
        const lines = source.value.substring(0, source.value.split('\n').slice(0, topLine + 1).join('\n').length).split('\n');
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      newSections.splice(sectionIndex, endIndex - sectionIndex, { ...section, lines: newSectionMd.split('\n') });
  ...
  function closeSectionEdit(wrapper) {
    ...
    document.querySelectorAll('.section-edit-btn').forEach(b => b.style.display = '');
    window.__editingMode = false;
    wrapper.remove();
  }
  ...
  const updateFullPreview = () => {
    if (window.marked) {
      ...
      editPreview.querySelectorAll('a[href]').forEach(a => { a.setAttribute('target', '_blank'); a.setAttribute('rel', 'noopener noreferrer'); });
    }
    clearTimeout(fullPreviewTimer);
    fullPreviewTimer = setTimeout(async () => {
      try {
        const res = await apiPost('/api/render', { content: editTextarea.value });
        if (res.html) {
  ...
  document.getElementById('btn-edit').addEventListener('click', async () => {
    document.querySelectorAll('.section-edit-area').forEach(el => closeSectionEdit(el));
    const res = await getContent();
  ...
  editTextarea.addEventListener('scroll', () => {
    ...
    fullSyncLock = true;
    const lineHeight = parseFloat(getComputedStyle(editTextarea).lineHeight) || 18;
    ...
    if (lastHeading) {
      ...
      for (const h of headings) {
        if (h.textContent.trim() === lastHeading) {
          ...
          requestAnimationFrame(() => { fullSyncLock = false; });
          return;
        }
      }
    }
    const ratio = editTextarea.scrollTop / (editTextarea.scrollHeight - editTextarea.clientHeight || 1);
    editPreview.scrollTop = ratio * (editPreview.scrollHeight - editPreview.clientHeight);
    requestAnimationFrame(() => { fullSyncLock = false; });
  });
  ...
  document.getElementById('btn-rename').addEventListener('click', async () => {
    ...
    const res = await apiPost('/api/rename', { old_path: oldPath, new_path: newPath });
    if (res.success) {
  ...
  document.getElementById('btn-new').addEventListener('click', async () => {
    const currentDir = pagePath.includes('/') ? pagePath.substring(0, pagePath.lastIndexOf('/') + 1) : '';
  ...
  document.getElementById('btn-history').addEventListener('click', async (e) => {
    e.stopPropagation();
    ...
    if (!data.backups || data.backups.length === 0) {
      historyPanel.innerHTML = '<div style="padding:8px;">' + t('noHistory') + '</div>';
      return;
    }
    ...
    data.backups.forEach(b => {
      ...
      row.appendChild(mkBtn(t('preview'), 'btn-edit', async () => {
        const bdata = await getBackup(b.timestamp);
        const current = await getContent();
        const html = buildDiffHtml(bdata.content || '', current.content || '', b.datetime);
        const win = window.open('', '_blank');
        win.document.write(html);
        win.document.close();
      }));
      row.appendChild(mkBtn(t('restore'), 'btn-save', async () => {
        if (!confirm(b.datetime + t('restoreConfirm'))) return;
        const res = await restoreBackup(b.timestamp);
        if (res.success) { alert(t('restored')); location.reload(); }
        else alert(t('restoreFailed'));
      }));
      row.appendChild(mkBtn(t('delete'), 'btn-cancel', async () => {
    ...
    delAllBtn.addEventListener('click', async () => {
      ...
      if (res.success) historyPanel.innerHTML = '<div style="padding:8px;">' + t('noHistory') + '</div>';
    });
  ...
  document.addEventListener('click', () => { historyPanel.style.display = 'none'; });
  ...
  function buildDiffHtml(oldContent, newContent, label) {
    ...
    let i=m, j=n;
    while (i>0||j>0) {
      if (i>0&&j>0&&oldLines[i-1]===newLines[j-1]) { diff.push({t:'s',v:oldLines[i-1]});i--;j--; }
      else if (j>0&&(i===0||dp[i][j-1]>=dp[i-1][j])) { diff.push({t:'a',v:newLines[j-1]});j--; }
      else { diff.push({t:'d',v:oldLines[i-1]});i--; }
    }
    diff.reverse();
    let html = '<div style="font-family:monospace;font-size:12px;padding:1rem;">';
    html += `<h3>${t('diffTitle', label)}</h3>`;
    html += '<table style="width:100%;border-collapse:collapse;">';
    html += '<tr><th style="width:50%;background:#f5f5f5;padding:4px;border:1px solid #ddd;">' + t('diffHistory') + '</th>';
    html += '<th style="width:50%;background:#f5f5f5;padding:4px;border:1px solid #ddd;">' + t('diffCurrent') + '</th></tr>';
    for (const d of diff) {
      const v = esc(d.v)||'&nbsp;';
      if (d.t==='s') html += `<tr><td style="padding:2px 4px;border:1px solid #ddd;">${v}</td><td style="padding:2px 4px;border:1px solid #ddd;">${v}</td></tr>`;
      else if (d.t==='d') html += `<tr><td style="padding:2px 4px;border:1px solid #ddd;background:#ffd7d5;">${v}</td><td style="padding:2px 4px;border:1px solid #ddd;"></td></tr>`;
      else html += `<tr><td style="padding:2px 4px;border:1px solid #ddd;"></td><td style="padding:2px 4px;border:1px solid #ddd;background:#d4f0d4;">${v}</td></tr>`;
    }
    html += '</table></div>';
    return html;
  }
  ...
  searchInput.addEventListener('input', () => {
    ...
    searchTimer = setTimeout(async () => {
      ...
      if (!data.results || data.results.length === 0) {
        ...
        searchResults.innerHTML = count + data.results.map(r => {
          const pathLabel = r.path.includes('/') ? r.path.substring(0, r.path.lastIndexOf('/')) : '';
  ...
  searchInput.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      items.forEach((item, i) => item.classList.toggle('search-selected', i === searchSelectedIndex));
  ...
  searchInput.addEventListener('input', () => { searchSelectedIndex = -1; });
  searchInput.addEventListener('blur', () => setTimeout(() => searchResults.style.display = 'none', BLUR_DELAY));
    ...
    draftKeys.forEach(headingText => {
      ...
      if (action) {
        ...
        for (const btn of btns) {
          if (btn.parentElement && getHeadingText(btn.parentElement) === headingText) {
  ...
  function initScrollTitle() {
    ...
    const pageTitle = parts[parts.length - 1] || parts[parts.length - 2] || pagePath;
    let showing = 'site';
  ...
  function initTocTracking() {
    ...
    const headings = [];
    document.querySelectorAll('#article-body h1, #article-body h2, #article-body h3, #article-body h4, #article-body h5, #article-body h6').forEach(el => {
    ...
    if (tocSidebar) {
      tocSidebar.addEventListener('mouseenter', () => { tocHovered = true; });
      tocSidebar.addEventListener('mouseleave', () => { tocHovered = false; });
    }
    let firstRun = true;
    const scrollTocTo = (li) => {
      ...
      if (liTop < sidebarTop + 60 || liBottom > sidebarBottom - 20) {
        const target = tocSidebar.scrollTop + (liTop - sidebarTop) - tocSidebar.clientHeight / 2 + li.offsetHeight / 2;
  ...
  function enableImageDrop(textarea) {
    ...
    overlay.style.cssText = 'position:absolute;inset:0;background:rgba(74,111,165,0.15);border:2px dashed #4a6fa5;border-radius:3px;z-index:10;display:none;align-items:center;justify-content:center;font-size:14px;color:#4a6fa5;pointer-events:none;';
    ...
    textarea.addEventListener('dragenter', (e) => {
      e.preventDefault(); e.stopPropagation(); dragCount++; overlay.style.display = 'flex';
    });
    textarea.addEventListener('dragleave', (e) => {
      e.stopPropagation(); dragCount--;
      if (dragCount <= 0) { overlay.style.display = 'none'; dragCount = 0; }
    });
    textarea.addEventListener('dragover', (e) => { e.preventDefault(); e.stopPropagation(); });
    textarea.addEventListener('drop', async (e) => {
      e.preventDefault(); e.stopPropagation(); overlay.style.display = 'none'; dragCount = 0;
  ...
  function initSidebarEdit() {
    ...
    editBtn.addEventListener('click', async () => {
      ...
      allItems.forEach(li => {
        ...
        const link = li.querySelector(':scope > .nav-link, :scope > .nav-dir-title');
        if (!link) return;
        ...
        li.addEventListener('dragend', () => {
          ...
          dragItem = null;
          sidebar.querySelectorAll('.nav-drop-above, .nav-drop-below, .nav-drop-into').forEach(el => {
            el.classList.remove('nav-drop-above', 'nav-drop-below', 'nav-drop-into');
          });
        ...
        li.addEventListener('dragover', (e) => {
          ...
          if (isDir && !hasChildren) {
            if (y < h * 0.20) li.classList.add('nav-drop-above');
            else if (y > h * 0.80) li.classList.add('nav-drop-below');
        ...
        li.addEventListener('dragleave', () => {
          li.classList.remove('nav-drop-above', 'nav-drop-below', 'nav-drop-into');
        });
        ...
        li.addEventListener('drop', (e) => {
          ...
          let action;
          if (isDir && !hasChildren) {
            if (y < h * 0.20) action = 'above';
            else if (y > h * 0.80) action = 'below';
            else action = 'into';
          } else if (isDir && y > h * 0.25 && y < h * 0.75) {
            action = 'into';
          } else if (y < h / 2) {
      ...
      const cleanup = () => {
        sidebar.querySelectorAll('.nav-edit-wrapper').forEach(w => w.remove());
        sidebar.querySelectorAll('.nav-link, .nav-dir-title').forEach(l => l.style.display = '');
        sidebar.querySelectorAll('.nav-collapse-toggle').forEach(t => t.style.display = '');
        sidebar.querySelectorAll('.nav-item[draggable]').forEach(li => li.removeAttribute('draggable'));
        saveRow.remove();
        editBtn.innerHTML = ICONS.gear;
        editing = false;
        // Re-apply collapsed state from localStorage
        try {
          const collapsed = JSON.parse(localStorage.getItem('gomdshelf-collapsed')) || {};
          sidebar.querySelectorAll('.nav-dir[data-key]').forEach(li => {
            const key = li.dataset.key;
            const childUl = li.querySelector(':scope > .nav-list');
            const toggle = li.querySelector(':scope > .nav-link .nav-collapse-toggle, :scope > .nav-dir-title .nav-collapse-toggle');
      ...
      saveBtn.addEventListener('click', async () => {
        ...
        sidebar.querySelectorAll('.nav-edit-input').forEach(input => {
          ...
          const currentParentLi = li.parentElement.closest('.nav-item[data-key]');
          const currentParent = currentParentLi ? currentParentLi.dataset.key : '';
          const originalParent = segments.length > 1 ? segments.slice(0, -1).join('/') : '';
        ...
        const hasChanges = Object.keys(renames).length > 0 || Object.keys(moves).length > 0;
        ...
        if (hasChanges) {
          ...
          setTimeout(() => { window.__editingMode = false; }, NAV_SAVE_GUARD);
        }
      });
    });
  }
  ...
  function initSidebarCollapse() {
    ...
    document.querySelectorAll('.sidebar-left .nav-dir').forEach(li => {
      const key = li.dataset.key;
      const titleEl = li.querySelector(':scope > .nav-link.nav-dir-title, :scope > .nav-dir-title');
      const childUl = li.querySelector(':scope > .nav-list');
  ...
  function initThemePicker() {
    ...
    const faviconColors = { blue: '%237aaed5', green: '%2381c784', red: '%23ef9a9a', yellow: '%23ffcc80' };
    const faviconColorsDark = { blue: '%23456a8f', green: '%23438a47', red: '%23a33030', yellow: '%23b06010' };
    function updateFavicon(theme) {
      ...
      if (favicon) {
        favicon.href = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><rect fill='" + fc + "' width='32' height='32' rx='6'/><path d='M6 7L6 25Q16 20 26 25L26 7Q16 12 6 7Z' fill='none' stroke='white' stroke-width='2' stroke-linejoin='round'/><path d='M16 10L16 23' stroke='white' stroke-width='1.2'/></svg>";
      }
    }
  ...
  function initHeadingLinks() {
    document.querySelectorAll('#article-body h1, #article-body h2, #article-body h3, #article-body h4, #article-body h5, #article-body h6').forEach(h => {
      if (!h.id) return;
      ...
      link.addEventListener('click', (e) => {
        ...
        else { const ta = document.createElement('textarea'); ta.value = url; ta.style.cssText = 'position:fixed;opacity:0'; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); document.body.removeChild(ta); }
  ...
  function initCheckboxes() {
    ...
    checkboxes.forEach((cb, index) => {
      ...
      cb.addEventListener('change', async () => {
        ...
        let md = res.content;
        let cbIndex = 0;
        md = md.replace(/^(\s*[-*+]\s+)\[([ xX])\]/gm, (match, prefix, check) => {
          if (cbIndex === index) {
            cbIndex++;
  ...
  function initExternalLinks() {
    document.querySelectorAll('#article-body a[href]').forEach(a => {
      ...
      if (href && (href.startsWith('http://') || href.startsWith('https://')) && !href.includes(location.host)) {
  ...
  function initRelativeTime() {
    ...
    else if (sec < 31536000) rel = Math.floor(sec / 2592000) + ' ' + t('monthAgo');
  ...
  window.addEventListener('beforeunload', () => {
    try {
      ...
      const allHeadings = document.querySelectorAll('#article-body h1, #article-body h2, #article-body h3, #article-body h4, #article-body h5, #article-body h6');
      let anchorId = '';
      ...
      sessionStorage.setItem('gomdshelf-scroll', JSON.stringify({
      ...
      const tocSidebar = document.querySelector('.sidebar-right');
      if (tocSidebar) {
        sessionStorage.setItem('gomdshelf-toc-scroll', JSON.stringify({
          path: location.pathname,
